...
import { UserRole } from "./clientController";
...
class AdminController {
  ...
  async UpdateLoggedInAdminDetails(
    req: UserRequestInterface,
    res: Response,
  ): Promise<void> {
    try {
      ...
      await sequelize.query(
        `UPDATE users SET name = ?, email = ?, phoneNumber = ?, DOB = ?, gender = ?, address = ?, updatedAt = ?  WHERE id = ?`,
        {
  ...
  async RegisterStaff(req: Request, res: Response): Promise<void> {
    ...
    console.log("emaul exist", isEmailExist);
    ...
    const [isPhoneExists] = await sequelize.query(
    ...
    //await sequelize.query(
    //  `INSERT INTO users(id, name, email, password, phoneNumber, profilePicture, DOB, gender, address, role, status, createdAt, updatedAt, branch) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    //  {
    ...
    const staff = new UserModel({
      ...
      profilePictureUrl: staffPhotoPath,
      DOB: dateOfBirth,
      gender,
      address,
      role: UserRole.STAFF,
  ...
  async FetchStaffList(_req: Request, res: Response): Promise<void> {
    try {
  ...
  async UpdateStaffDetails(req: Request, res: Response): Promise<void> {
    try {
      ...
      // await sequelize.query(
      //   `UPDATE users SET name = ?, email = ?, phoneNumber = ?, DOB = ?, gender = ?, address = ?, status = ?, updatedAt = ?  WHERE id = ?`,
      //   {
      //     type: QueryTypes.UPDATE, // Define query type as UPDATE
      //     replacements: [
      //       name,
  ...
  async DeleteStaff(req: Request, res: Response): Promise<void> {
    try {
      ...
      const [oldStaffPhotoPath]: any = await sequelize.query(
  ...
  async FetchCustomerList(_req: Request, res: Response): Promise<void> {
    try {
      // Query the database to get all users with the role "Customer"
      const CustomerList = await sequelize.query(
        `SELECT * FROM users WHERE role = ?`,
        {
          type: QueryTypes.SELECT, // Define query type as SELECT
  ...
  async DeleteCustomerDetails(req: Request, res: Response): Promise<void> {
    try {
      ...
      const [oldCustomerPhotoPath]: any = await sequelize.query(
        `SELECT profilePicture FROM users WHERE id = ?`,
        {
          type: QueryTypes.SELECT,
  ...
  async RegisterBranch(req: Request, res: Response): Promise<void> {
    ...
    // Check if the email already exists in the database
    const [isEmailExist] = await sequelize.query(
      `SELECT email FROM branches WHERE email = ?`,
      {
        type: QueryTypes.SELECT,
        replacements: [email],
      },
    ...
    // Check if the phone number already exists in the database
    const [isPhoneExists] = await sequelize.query(
      `SELECT phone FROM branches WHERE phone = ?`,
      {
        type: QueryTypes.SELECT,
        replacements: [phone],
      },
    ...
    await sequelize.query(
      `INSERT INTO branches(id, branchName, email, phone, address, latitude, longitude, createdAt, updatedAt) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      {
        type: QueryTypes.INSERT,
        replacements: [
          id,
  ...
  async UpdateBranchDetails(req: Request, res: Response): Promise<void> {
    try {
      ...
      const { branchName, email, phone, address, latitude, longitude } =
        req.body;
      const time = new Date(); // Get the current timestamp for updatedAt field
      ...
      const [isBranchExists]: any = await sequelize.query(
        `SELECT * FROM branches WHERE id = ?`,
        {
          type: QueryTypes.SELECT,
      ...
      await sequelize.query(
        `UPDATE branches SET branchName = ?, email = ?, phone = ?, address = ?, latitude = ?, longitude = ?, updatedAt = ?  WHERE id = ?`,
        {
          type: QueryTypes.UPDATE, // Define query type as UPDATE
          replacements: [
            branchName,
  ...
  async RegisterService(req: Request, res: Response): Promise<void> {
    // Cast the request to include Multer file properties
    const multerReq = req as unknown as MulterRequest;
    const { service_name, description, service_design_type, duration, price } =
      req.body;
    ...
    if (servicePhoto) {
      ...
      // Check if all required fields are provided
      if (!service_name || !description || !price || !Number(duration)) {
    ...
    const [isServiceExist] = await sequelize.query(
    ...
    // await sequelize.query(
    //   `INSERT INTO services(id, service_name, description, photo, createdAt, updatedAt, service_design_types) VALUES(?, ?, ?, ?, ?, ?, ?)`,
    //   {
    //     type: QueryTypes.INSERT,
    //     replacements: [
    //       id,
    ...
    await ServiceModel.create({
      service_name,
      description,
      photo: servicePhotoPath,
      service_design_types: service_design_type,
      price: Number(price),
  ...
  async UpdateService(req: Request, res: Response): Promise<void> {
    try {
      ...
      const [oldServicePhotoPath]: any = await sequelize.query(
        `SELECT photo FROM services WHERE id = ?`,
        {
          type: QueryTypes.SELECT,
      ...
      if (servicePhoto) {
        // Modify the file path to store the correct URL
        const cutServicefFileURL = servicePhoto.substring(
          "src/uploads/".length,
        );
        const newServiceFilePath = process.env.HOST_PATH + cutServicefFileURL;
        servicePhotoPath = newServiceFilePath;
      ...
      const [isServiceExists]: any = await sequelize.query(
        `SELECT * FROM services WHERE service_name = ? AND id != ?`,
        {
          type: QueryTypes.SELECT,
      ...
      await sequelize.query(
        `UPDATE services SET service_name = ?, description = ?, photo = ?, updatedAt = ?  WHERE id = ?`,
        {
          type: QueryTypes.UPDATE, // Define query type as UPDATE
          replacements: [
            service_name,
  ...
  async RegisterAppoinemtService(req: Request, res: Response): Promise<void> {
    try {
      ...
      const [isAppointmentExist] = await sequelize.query(
        `SELECT * FROM appointment_services WHERE branchId = ? AND serviceId = ?`,
        {
          type: QueryTypes.SELECT,
      ...
      await sequelize.query(
        `INSERT INTO appointment_services(id, appointmentStatus, createdAt, updatedAt, branchId, serviceId ) VALUES(?, ?, ?, ?, ?, ?)`,
        {
          type: QueryTypes.INSERT,
          replacements: [
            id,
  ...
  async FetchAppointmentServiceList(
    req: Request,
    res: Response,
  ): Promise<void> {
    try {
      // Query the database to get all appointment services Join query to branches and services
      const appointmentServicesList = await sequelize.query(
        `SELECT a.id, a.appointmentStatus, b.id AS branchId, b.branchName, b.address, s.id AS serviceId, s.service_name, s.duration , s.price  FROM appointment_services a JOIN services s ON a.serviceId = s.id JOIN branches b ON a.branchId = b.id`,
        {
          type: QueryTypes.SELECT,
        },
      );
      ...
      res.status(500).json({
        message:
          "Internal server error while fetching appointment service list",
      });
    }
  }

  // Fetch details of a single appointment service by their ID
  async FetchSingleAppointmentServiceDetails(
    req: Request,
    res: Response,
  ): Promise<void> {
    try {
      ...
      const [appointmentServiceDetails] = await sequelize.query(
        `SELECT * FROM appointment_services WHERE id = ?`,
        {
          type: QueryTypes.SELECT, // Define query type as SELECT
      ...
      res.status(500).json({
        message:
          "Internal server error while fetching appointment service details",
      });
    }
  }

  // Fetch booking list from the database
  async FetchBookings(_req: Request, res: Response): Promise<void> {
    try {
  ...
  async DeleteBooking(req: Request, res: Response): Promise<void> {
    try {
      ...
      const leftBookings = await BookingModel.findAll({
        include: [
          {
            model: UserModel,
            as: "staff",
  ...
  async UpdateAppointmentServiceDetails(
    req: Request,
    res: Response,
  ): Promise<void> {
    try {
      ...
      const [isAppointmentServiceExists]: any = await sequelize.query(
        `SELECT * FROM appointment_services WHERE id = ?`,
        {
          type: QueryTypes.SELECT,
      ...
      await sequelize.query(
        `UPDATE appointment_services SET branchId = ?, serviceId = ?, appointmentStatus = ?, updatedAt = ?  WHERE id = ?`,
        {
          type: QueryTypes.UPDATE, // Define query type as UPDATE
          replacements: [
            branchId,
      ...
      res.status(500).json({
        message:
          "Internal server error while updating appointment service details",
      });
    }
  }

  async FetchPayments(_req: Request, res: Response): Promise<void> {
    try {
